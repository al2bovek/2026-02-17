...
import {sql,  testConnection } from "./dbConnection.js";
...
(async () => {
  try {
    ...
    //start server
    app.listen(port, () => {
      console.log(`Server is ready and listening port ${process.env.DB_PORT}`);
    });
  } catch (error) {
...
process.on("SIGINT", async () => {
  console.log(`Closing database: ${process.env.DB_NAME} connection`);
